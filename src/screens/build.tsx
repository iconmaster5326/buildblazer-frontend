...
function InfoTab({ build }: { build: Build }) {
  ...
  const [_, setName] = useState(build.name);
...
const ReminderText = styled(SizableText, {
  textAlign: "center",
  fontStyle: "italic",
});
          ...
          <ContextMenu.Trigger asChild>
            <YGroup>
              {milestones.map((milestone, index) => (
                ...
                >
                  <ListItem>{milestone.name}</ListItem>
                </YGroup.Item>
              ))}
            </YGroup>
          </ContextMenu.Trigger>
    ...
    <YStack gap="$4">
      {sheets.length === 0 ? (
        <ReminderText>
          ...
          you can track their state and roll using thier statistics. Press
          &quot;{media.md ? "Add Sheet" : "+"}&quot; to add a new sheet!
        </ReminderText>
      ) : (
        <ContextMenu>
          <ContextMenu.Trigger asChild>
      ...
      {media.md ? (
        <Button onPress={addSheet}>Add Sheet</Button>
      ) : (
        <PlusButton onPress={addSheet} />
      )}
    </YStack>
  );
}

export default function ScreenBuild({ route }: any) {
  const build = route.params.build as Build;
  const media = useMedia();
